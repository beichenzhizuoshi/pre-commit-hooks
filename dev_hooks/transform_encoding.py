...
def _execute_transform_action(options: argparse.Namespace) -> int:
  ...
  if not output_encoding:
    logging.error('Output file encoding not supported: %s', options.encoding)
    return 0
  ...
  for file in options.filenames:
    ...
    if not input_attr:
      logging.error('Input file encoding not supported: %s', input_encoding)
      continue
...
please manually repair the file code or forcefully delete the file: %s', yaml_config)
    ...
    changed = _transform_file_encoding(path, input_encoding, output_encoding)
    if changed:
      logging.info('transform encoding ==> %s, from: %s to: %s', path, input_encoding, output_encoding)
      ret += 1
...
def _execute_chinese_transform_action(options: argparse.Namespace) -> int:
  ...
  for file in options.filenames:
    ...
    if _transform_chinese_file_encoding(path, input_encoding, output_encoding):
      ret += 1
      logging.info('transform chinese encoding ==> %s, from: %s, to: %s', path, input_encoding, output_encoding)
  return ret
...
def _init_arguments():
  ...
  parser.add_argument('-e', '--encoding', default='utf-8', help='Set automatic conversion file encoding.')
  parser.add_argument('-v', '--verbose', action='store_true', help='Show more output.')
  parser.add_argument('-r', '--restore', action='store_true', help='Automatically restore file encoding.')
  parser.add_argument('-f', '--force', action='store_true', help='Coercion encoding is not recoverable.')
  parser.add_argument('--chinese', action='store_true', help='Convert file encodings containing chinese.')
  parser.add_argument(
